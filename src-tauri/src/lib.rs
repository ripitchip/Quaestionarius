...
fn save_credentials(app: AppHandle, file_path: String) -> Result<String, String> {
    ...
    let file_exists = saved_path.exists();

    fs::write(&saved_path, credentials_content)
        .map_err(|e| format!("Failed to save credentials: {}", e))?;

    let result = serde_json::json!({
        "status": if file_exists { "updated" } else { "saved" },
...
#[tauri::command]
fn authenticate_google(app: AppHandle) -> Result<String, String> {
    ...
    let credentials_content = fs::read_to_string(&saved_path)
